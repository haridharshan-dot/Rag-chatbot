...
export default function ChatWidget({ sessionId, studentId, loading, defaultOpen = false, hideFab = false }) {
  ...
  useEffect(() => {
    const closedDimensions = { width: 118, height: 86 };
    ...
    const openDimensions = isSmallScreen
      ? {
          width: Math.max(300, viewportWidth - 16),
          height: Math.round(Math.max(520, viewportHeight * 0.84)),
        }
      : {
          width: Math.max(360, Math.min(460, viewportWidth - 32)),
          height: Math.round(Math.max(620, Math.min(760, viewportHeight * 0.9))),
        };
